	...
	function GetByID(id){
        ...
        var settings = {
			...
			$.ajax(settings).done(function (response) {
				$('#Id').val(response.Id);
				$('#Nome').val(response.Registro);
				$('#Login').val(response.Titulo);
				$('#Senha').val(response.Isbn);
				$('#Email').val(response.Genero);
				$('#Senha').val(response.Sinopse);
				$('#Senha').val(response.Observações);
